...
public abstract class ContentProviderWithDecorator extends ContentProvider implements ContentProviderDecorator {
    ...
    public synchronized void startBatchOperation(Uri... uris) {
    ...
    public synchronized void endBatchOperation(Uri... uris) {
    ...
    public synchronized void endBatchOperationWithoutNotification(Uri... uris) {
        if (mMonitoringUris.size() > 0) {
            UriParams uriParams;
            for (Uri uri: uris) {
                if ((uriParams = getUriParams(uri)) != null) {
