...
public class SmsUtils extends ContentObserver {
    ...
    public SmsUtils(ContentResolver resolver, Handler handler) {
        super(handler);
        mResolver = resolver;
    }
    ...
    public void onChange(boolean selfChange) {
        Log.e("===", "onChange() ,selfChange:" + selfChange);
        onReceiveSms();
    }
    ...
    private void onReceiveSms() {
        ...
        try {
            ...
            if (cursor != null) {
                ...
                // 得到短信号码和内容之后进行相关处理
                Log.e("===", "address:" + strAddress + ",body:" + strbody);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private static SmsUtils utils;

    public static void registerSmsDatabaseChangeObserver(ContextWrapper contextWrapper) {
        //因为，某些机型修改rom导致没有getContentResolver
        try {
            Log.e("===", "registerSmsDatabaseChangeObserver");
            utils = new SmsUtils(contextWrapper.getContentResolver(), new Handler());
            contextWrapper.getContentResolver().registerContentObserver(SMS_MESSAGE_URI,
                    true, utils);
        } catch (Throwable b) {
            Log.e("===", b.toString());
        }
    ...
    public static void unregisterSmsDatabaseChangeObserver(ContextWrapper contextWrapper) {
        Log.e("===", "unregisterSmsDatabaseChangeObserver");
        try {
            contextWrapper.getContentResolver().unregisterContentObserver(utils);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
